...
sm_fun_param_obj *sm_new_fun_param_obj(sm_string *name, sm_object *default_val,
  ...
  self->content.default_val = default_val;
  self->content.known_type  = known_type;
  return self;
}
...
int sm_fun_param_sprint(sm_fun_param *self, char *buffer) {
  ...
  if (self->default_val != NULL) {
    buffer[cursor++] = '=';
    cursor += sm_object_sprint(self->default_val, buffer);
  }
  return cursor;
}

// return value hypothetical call to sm_fun_param_sprint
int sm_fun_param_to_string_len(sm_fun_param *self) {
  unsigned int cursor = self->name->size;
  if (self->default_val != NULL) {
    cursor += sm_object_to_string_len(self->default_val) + 1;
  }
  return cursor;
}
...
unsigned int sm_fun_to_string_len(sm_fun *self) {
  ...
  for (unsigned short int i = 0; i < self->num_params; i++) {
