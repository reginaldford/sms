...
struct sm_context *sm_new_context(unsigned int capacity){
  sm_context *new_context = (sm_context*)sm_malloc(sizeof(sm_context)+sizeof(sm_table_entry)*capacity);
...
sm_table_entry *get_table_entries(sm_context *context){
	return (sm_table_entry*) &(context[1]);
}
...
void sm_print_table(sm_context * context)
{
  ...
  {
  	sm_object *value =(sm_object*)(table_entries[i].value);
...
sm_context * sm_set_var(sm_context * context,sm_string *name, void *val)
{
	...
	if(current_context -> size == current_context -> capacity)
  {
    ...
    current_context = (sm_context*) sm_realloc((sm_object*)current_context,sizeof(sm_context)+sizeof(sm_table_entry) * new_capacity);
