...
uint32_t sm_object_sprint(sm_object *obj1, char *buffer, bool fake) {
  ...
  case SM_FF_SIG_TYPE:
    ...
    break;
  case SM_FF_TYPE:
    len += sm_ff_sprint((sm_ff *)obj1, buffer, fake);
    break;
  default: {
    if (!fake)
...
uint32_t sm_sizeof(sm_object *obj1) {
  ...
  case SM_STRING_TYPE:
    // We subtract 7 bytes because the sm_string has 7 filler bytes
    return sm_round_size64(sizeof(sm_string) - 7 + ((sm_string *)obj1)->size);
  case SM_SYMBOL_TYPE:
  ...
  case SM_FF_TYPE:
    return sizeof(sm_ff) + sizeof(ffi_type *) * ((sm_ff *)obj1)->cif.nargs;
  case SM_FF_SIG_TYPE:
    return sizeof(sm_ff_sig) + sizeof(ffi_type *) * ((sm_ff_sig *)obj1)->cif.nargs;
  default:
    return 0;
  }
}
