...
sm_node *sm_node_get_leaf(sm_node *root, char *needle, int32_t len);
/// Set the value of the leaf node addressed by value
void sm_node_set(sm_node *node, int32_t which, sm_object *value);
...
int32_t sm_node_map_left_count(uint64_t map, int32_t map_index);
...
int32_t sm_node_map_size(uint64_t map);
