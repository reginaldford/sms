...
sm_stack_obj *sm_new_stack_obj(uint32_t capacity) {
  sm_stack_obj *new_stack = sm_malloc(sms_heap,sizeof(sm_stack_obj) + sizeof(sm_object *) * capacity);
...
sm_stack_obj *sm_stack_obj_push(sm_stack_obj *self, void *ptr) {
  ...
  if (sm_stack_obj_size(self) == self->capacity) {
    ...
    current_stack     = sm_malloc(sms_heap,sizeof(sm_stack_obj) + sizeof(void *) * self->capacity);
