...
uint64_t sm_bytelength_parse(char *str, int32_t length) {
  ...
  for (int32_t i = 0; i < 32; i++) {
    ...
    if (sm_is_digit(current_char) || current_char == '.')
      ...
      int32_t unit          = sm_is_unit(current_char);
      int32_t unit_in_bytes = pow(1024, unit);
      buffer[i]             = 0;
