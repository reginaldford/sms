...
$(SRC_DIR0)/y.tab.c: $(SRC_DIR1)/parser/sms.y
	bison -d $(SRC_DIR1)/parser/sms.y -o $(SRC_DIR0)/y.tab.c

# Flex generates the lexer
$(SRC_DIR0)/lex.yy.c: $(SRC_DIR0)/y.tab.c $(SRC_DIR1)/parser/sms.l
...
# For developers
dev:
	$(MAKE) $(SRC_DIR0)/y.tab.c
	$(MAKE) $(SRC_DIR0)/lex.yy.c
	$(MAKE) -j$(THREADS) bin/$(TARGET_SMS) bin/$(TARGET_SMS_DBG) bin/$(TARGET_TEST) bin/$(TARGET_KT)
