...
sm_object * fib(sm_object * input){
  ...
  if(array->size > 2){
    printf("There should be 2 inputs: the fib sequence index, and a place for the output (a number).\n")
    return input;
  }
  sm_object** ptr_array = (sm_object**)(array+1);
  sm_f64 * input_number = (sm_f64*)(ptr_array[0]);
  sm_f64 * output = (sm_f64*)(ptr_array[1]);
  if(input_number->my_type != SM_F64_TYPE || n1 -> my_type != SM_F64_TYPE){
    printf("Both values given to fib must be floating point numbers.\n");
  }
  output->value = fib_recursive(input_number->value);
  return input;
}
